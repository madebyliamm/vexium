...
const FROM = 'Vertly <hello@vexium.ai>';
...
Deno.serve(async (_req) => {
  ...
  for (const user of (reminderUsers || [])) {
    ...
    await sendEmail(user.email, `Your Vertly trial ends tomorrow`, reminderHtml(name));
  ...
  for (const user of (expiredUsers || [])) {
    const name = user.display_name || user.username || 'there';
    if (!user.email) continue;
    await sendEmail(user.email, `Your Vertly free trial has ended`, expiredHtml(name));
...
function reminderHtml(name: string) {
  ...
  <p style="font-size:15px;color:#888;line-height:1.7;margin:0 0 28px">You have 1 day left on your Vertly free trial. Your projects stay exactly as they are — you just need a plan to keep building.</p>
...
function expiredHtml(name: string) {
  ...
  <p style="font-size:15px;color:#888;line-height:1.7;margin:0 0 8px">Your 7-day Vertly trial is up. Everything you built is still here — pick a plan to get back to it.</p>
